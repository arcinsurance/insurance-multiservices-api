...
export async function getIncomeSourcesByClientId(clientId: string) {
  const [rows]: [any[], any] = await db.query('SELECT * FROM income_sources WHERE client_id = ?', [clientId]);
  return rows;
}
...
export async function createIncomeSourceForClient(clientId: string, income: any) {
  const [result]: [any, any] = await db.query(
    `INSERT INTO income_sources (client_id, employer, position, annual_income) VALUES (?, ?, ?, ?)`,
    [
      clientId,
      income.employer,
      income.position,
...
export async function updateIncomeSourceForClient(clientId: string, income: any) {
  const [result]: [any, any] = await db.query(
    `UPDATE income_sources SET employer=?, position=?, annual_income=? WHERE client_id=?`,
    [
      income.employer,
      income.position,
      income.annual_income,
